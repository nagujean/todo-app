  ...
  persist(
    (set, get) => ({
      ...
      addTodo: async ({ title, description, startDate, endDate, priority }) => {
        ...
        logger.info('[addTodo] Adding todo:', { title: trimmedTitle, userId, teamId: currentTeamId, mode })
      ...
      toggleTodo: async (id) => {
        ...
        if ((userId || currentTeamId) && db) {
          try {
            ...
            logger.info('Todo toggled:', { id, newCompleted, mode: currentTeamId ? 'team' : 'personal' })
          } catch (error) {
      ...
      clearCompleted: async () => {
        ...
        if ((userId || currentTeamId) && db) {
          try {
            ...
            await batch.commit()
            logger.info('Cleared completed todos:', { count: completedTodos.length, mode: currentTeamId ? 'team' : 'personal' })
          } catch (error) {
